describe Stompede::Stomp::Message do
  describe "#content_length" do
    ...
    it "raises an error if invalid content length defined" do
      ...
      expect { message.content_length }.to raise_error(Stompede::Stomp::Error, /invalid content length "LAWL"/)
    end
  end
  ...
  describe "#to_str" do
    ...
    specify "overrides user-specified content-length" do
      message = Stompede::Stomp::Message.new("CONNECT", { "content-length" => "10" }, "\x00ab\x00")
      message.to_str.should eq "CONNECT\ncontent-length:4\n\n\x00ab\x00\x00"
    end
  end
end
